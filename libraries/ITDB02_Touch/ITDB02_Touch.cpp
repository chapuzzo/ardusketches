...
void ITDB02_Touch::InitTouch(byte orientation)
{
    ...
    pinModeFast(T_CLK,  OUTPUT);
    pinModeFast(T_CS,   OUTPUT);
    pinModeFast(T_DIN,  OUTPUT);
    pinModeFast(T_DOUT, INPUT);
    pinModeFast(T_IRQ,  INPUT);

    digitalWriteFast(T_CS,  HIGH);
    digitalWriteFast(T_CLK, HIGH);
    digitalWriteFast(T_DIN, HIGH);
    digitalWriteFast(T_CLK, HIGH);
}
