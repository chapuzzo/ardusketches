...
void setup() { //initial setup
  ...
  lcd.baudChange(1000000);                                      // Set a fast baud!, always that we use touch functions is recommended to use fast baud rates
    //Serial.println("lcd-baudchange");
...
void loop() { //main loop
    ...
    lcd.string(30,160,310,220,MAGENTA,FONT4,TRANS,"Touch screen to begin");
      //Serial.println("strings printed");
    while(!lcd.touchScreen(touch));                                     // Wait for a touch on the screen to start
      //Serial.println("lcd-touched");
    lcd.erase();
      //Serial.println("lcd-erased");
    delay(500);
    ...
    while(1){                                                           // Loop forever
      ...
      if(lcd.touchIcon(icon)){                                          // If we receive a touch on any icon
        ...
        lcd.string(30,160,310,220,MAGENTA,FONT4,TRANS,icon);
      }
      delay(15);
      sprintf(ml,"%9ld",millis());
      lcd.drawRectangle(30,190,200,220,BLACK,FILL);
      lcd.string(30,190,200,220,MAGENTA,FONT4,TRANS,ml);
    }
}
